 ...
 */
function buildLink(longname, linkText, options, keepOpen = false) {
    ...
    if (!fileUrl) {
        ...
        var linkHtml = `<a href="${encodeURI(fileUrl + fragmentString)}"${classString}>${text}`;
        if (!keepOpen) linkHtml += '</a>';
        return linkHtml;
    }
}
 ...
 */
var linkto = exports.linkto = function(longname, linkText, cssClass, fragmentId, keepOpen) {
