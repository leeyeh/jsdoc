        ...
        if ( parents && (doc.kind === 'class' || doc.kind === 'interface') ) {
            ...
            for (var j = 0, jj = parents.length; j < jj; j++) {
                ...
                for (var k = 0, kk = members.length; k < kk; k++) {
                    ...
                    var parentMember = members[k];
                    if (parentMember.meta && parentMember.meta.code && parentMember.meta.code.node && parentMember.meta.code.node.value && parentMember.meta.code.node.value.async) {
                        member.meta.async = true;
                    }
